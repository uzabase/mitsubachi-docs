...
> mockup で再現する場合は `mockup-kit/mitsubachi-mockup.css` の `.mi-suggestion`（+ `__category`/`-item`/`__empty`） を使う（自作しない）。 組む前に見本 `mockup-kit/components/suggestion.html` を読んでマークアップ構造を踏襲する。
