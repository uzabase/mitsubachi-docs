...
> mockup で再現する場合は `mockup-kit/mitsubachi-mockup.css` の `.mi-menu-item--sub` を使う（自作しない）。 組む前に見本 `mockup-kit/components/menu.html` を読んでマークアップ構造を踏襲する。sub-menu は原則**親 menu の右側に上端を揃えて**表示（右にスペースが無ければ左でもよい。下に無ければ上に伸ばしてよい）。開閉は hover（タッチデバイスは Tap トグル・メニュー外 Tap で全て閉じる）。
