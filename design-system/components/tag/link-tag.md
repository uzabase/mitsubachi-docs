...
link-tag を「いつ使うか」「いつ使わないか」、また似たコンポーネントとの使い分けを示します。
...
- クリック／タップ: 関連コンテンツページへ遷移します。
- ホバー: hover 状態になり、視覚的なフィードバックを提供します。
- フォーカス: キーボード操作でフォーカスが当たると focus 状態を表示します。
- アクティブ: クリック中に active 状態を表示します。
...
- **解決する課題**: コンテンツのカテゴリや分類をクリック可能なラベルで表現し、関連コンテンツへの導線を提供します。
- **UI上での基本的な役割**: コンテンツに付与されたタグをリンクとして表示します。
- **ユーザーにとっての意味**: タグをクリックすることで、同じカテゴリや分類のコンテンツ一覧へ移動できます。
