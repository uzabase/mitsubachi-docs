...
> mockup で再現する場合は `mockup/mitsubachi-mockup.css` の `.mi-link-tag`（並べて使う） を使う（自作しない）。
...
- タグは横方向に並んで表示されます。
- Show More が有効な場合、表示しきれないタグは非表示になり、「もっと見る」ボタンをタップすることで残りのタグが表示されます。
- 各 link-tag のクリック／タップはそれぞれ関連コンテンツへ遷移します。
...
- **解決する課題**: 複数の link-tag を個別に配置する手間を統一化されたコンポーネントで解決します。
- **UI上での基本的な役割**: コンテンツに付与された複数のナビゲーションリンク型タグをまとめて表示します。
- **ユーザーにとっての意味**: コンテンツに関連する複数のカテゴリを一目で確認できます。
