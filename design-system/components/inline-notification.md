...
> mockup で再現する場合は `mockup-kit/mitsubachi-mockup.css` の `.mi-inline-notification--error/--information/--success/--warning` を使う（自作しない）。グレー面で控えめに見せる secondary には `.mi-inline-notification--secondary`、スマートフォン向けには `.mi-inline-notification--phone` を付ける。
