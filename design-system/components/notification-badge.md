...
### 幅（Figma node 10748-4969 の実測）

| 表示 | サイズ |
|---|---|
| ドット | **8×8px** |
| 1桁（1〜9） | **16×16px** |
| 2桁（10〜99） | **16×16px**（1桁と同じ） |
| 3桁（99+） | **22×16px** |

> **配置の注意**: 数値表示は**桁数によって幅が変わる**ため、重ねる位置の基準（右端を揃えるのか中心を揃えるのか）を
> 先に決めておく。Figma も「基準とする位置を決めておいてください」と明記している。
> kit の `.mi-badge-anchor` は**右端基準**（親の右上角から `translate(40%, -20%)`）。
