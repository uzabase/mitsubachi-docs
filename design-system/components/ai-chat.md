...
> mockup で再現する場合は `mockup-kit/mitsubachi-mockup.css` の `.mi-ai-chat`（+ `__messages`/`__user-message`/`__answer`/`__input`/`__disclaimer`） を使う（自作しない）。 組む前に見本 `mockup-kit/components/ai-chat.html` を読んでマークアップ構造を踏襲する。
