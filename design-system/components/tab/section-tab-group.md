...
section-tab-group は [section-tab](./section-tab.md) を内包する親コンポーネントです。[section-tab](./section-tab.md)（個々のタブアイテム）と section-tab-group（タブをまとめたグループ）の2つのコンポーネントで、section-tab の機能全体が構成されます。
...
- タブが選択されると、対応するセクションコンテンツが表示されます
- キーボードのTabキーでフォーカスを移動し、EnterキーまたはSpaceキーでタブを選択できます
- phone バリアントでは、選択中のタブが表示されるよう自動スクロールします
...
- section-tab の並びを管理し、セクションレベルのナビゲーション構造を形成します
- グループ内で常に1つのタブが選択された状態を管理します
- ページの構造や文脈を維持したまま、セクション内のコンテンツ切り替えを実現します
