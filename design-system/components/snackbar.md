...
snackbar は以下の要素で構成されます。

| 要素 | 説明 |
|---|---|
| **ステータスアイコン** | 操作の成功を示すアイコンです |
| **メッセージテキスト** | 操作結果を端的に伝えるテキストです |
| **閉じるボタン（icon-button）** | ユーザーが手動で snackbar を閉じるためのボタンです |
...
## 振る舞い
- **自動消去**: 表示後、一定時間が経過すると自動的に非表示になります
- **手動クローズ**: 閉じるボタン（icon-button）を押すことで、自動消去を待たずに手動で閉じることができます
- **レイアウトへの影響**: Overlay として既存 UI の上に重ねて表示されます。ページのコンテンツレイアウトには影響しません
...
## 役割と目的
- **解決する課題**: ユーザーが行った操作の結果を、操作フローを中断させずに短時間だけ伝える
- **UI上での基本的な役割**: 操作の成功を端的に示し、ユーザーの次の操作を妨げずに状態を共有します。[banner](./banner.md) よりも情報密度が低く、確実に見てもらうことを前提とせず、軽微な状態共有に特化します
- **ユーザーにとっての意味**: 「操作が成功した」ことをすばやく、邪魔なく確認できる
