...
> mockup で再現する場合は `mockup-kit/mitsubachi-mockup.css` の `.mi-snackbar`（画面隅スタックは `.mi-snackbar-viewport`）を使う（自作しない）。size は既定が medium、テキストが折り返さない短いメッセージには `.mi-snackbar--small` を付ける。

## 表示位置と挙動（Figma node 5607-6067 の仕様）

| 項目 | 値 |
|---|---|
| 表示位置（desktop） | 画面の**右上**。固定ヘッダー等の下に、上・右とも **16px** 空ける |
| 表示位置（phone） | 画面の**下中央**。下 **24px** / 左右 **16px** |
| アニメーション | desktop は右上からスライドイン / phone は下中央からスライドイン |
| 自動で消えるまで | **5秒以上**（目安）。閉じるボタンでも閉じられる |
| 複数表示 | 重ならないように縦に並べる（**16px** 間隔）。先頭が消えたら詰める |
| 幅 | medium は **min 280px**（文言が短くても確保）／ max **400px**。small は min なし（文言の長さ） |
| 高さ | max **88px**（3行ぶん）。**文言は2行以内**に収める |
| 文言 | 句点「。」読点「、」を省略しない。体言止めではなく「ですます調」 |
