...
日付選択・アコーディオン・グラフなどは kit にありません。**勝手に作らず、次の順で対応します。**

1. 既存の部品で代替できないか検討する（[component-selection.md の 9.](./component-selection.md)）
2. それでも作る場合は、**直前に申告を書く**

   ```html
   <!-- ds-exception: date picker は mitsubachi-ui に無いため代替実装 -->
   ```

   申告があれば層2の指摘は出ず、層3のレビュー材料に載ります（申告の無い逸脱は error）。
