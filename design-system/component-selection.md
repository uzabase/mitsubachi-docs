...
1. **既存の部品で代替できないか先に考える**（例: アコーディオン → card ＋ icon-button(chevron)、グラフ → table）
2. それでも必要なら**発明したことを申告する**。HTML の直前に `<!-- ds-exception: 理由 -->` を書く
3. `python3 tools/check-mockup.py <ファイル>` を実行し、申告漏れが無いことを確認する（未申告の逸脱は error になる）

> 代替の作り方（レシピ）は今後整備する。現時点では「勝手に作らない・作ったら申告する」を守る。
