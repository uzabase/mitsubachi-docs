...
| # | 項目 | 内容 | 判断が必要な点 |
|---|---|---|---|
| **A-5** | チャート／データ可視化のルール | データビズ用のカラーパレット（トークン準拠）＋ CSS だけで組めるバー・スパークライン・ドーナツの見本 | Speeda は経済情報プラットフォームでプロトにグラフがほぼ必ず出るのに kit にゼロ。AI は Chart.js を引くか独自色の SVG を描き、**そこだけ世界観が壊れる** |
...
| **hook** | セルフチェックの自動実行 | Claude Code の PostToolUse hook で、`.html` を Write / Edit したら `check-mockup.py` を自動実行する | チャットで頼めば実行できるので hook は「保険」。**AI が手順を飛ばす事故**（icon-button の selected が他環境で再現されなかった件と同型）を防げるが、Claude Code 限定で Cursor / Claude.ai には効かない |
